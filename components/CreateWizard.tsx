...
export function CreateWizard({initialModelId=''}:{initialModelId?:string}) {
  ...
  return (
    <div className="wizard-card">
      ...
      <div className="wizard-actions"><button className="button secondary" disabled={step === 1 || saving} onClick={() => setStep(step - 1)}>Back</button>{step < 7 ? <button className="button primary" disabled={(step === 2 && form.name.length < 2) || (step === 3 && !form.modelId) || (step === 5 && form.personality.length < 2)} onClick={() => setStep(step + 1)}>Continue</button> : <button className="button primary" disabled={saving || !form.name || !form.modelId || !form.interests || !form.biography} onClick={release}>{saving ? "Opening the town gate…" : "Release into the town"}</button>}</div>
    </div>
  );
}
