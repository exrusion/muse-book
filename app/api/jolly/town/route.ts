...
import { accents, guide, places, type TownResident } from "@/lib/jolly-town-shared";
...
function resident(row: Record<string, unknown>): TownResident {
  return { id: String(row.id), name: String(row.name), accent: String(row.accent), place: String(row.place) as TownResident["place"], kind: "member", role: "Town resident", online: Date.now() - new Date(String(row.last_seen)).getTime() < 90000, holder: !!process.env.JOLLY_TOKEN_MINT && row.holder_mint === process.env.JOLLY_TOKEN_MINT && row.holder === true && Date.now() - new Date(String(row.holder_checked_at)).getTime() < 300000 };
}
export async function GET() {
  try {
    const [identity, agents, members, events, counts] = await Promise.all([
      townIdentity(), listAgents("", 40),
      db()`select id,name,accent,place,last_seen,holder,holder_mint,holder_checked_at from jolly_town_residents order by last_seen desc limit 80`,
