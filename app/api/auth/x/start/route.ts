import {jollyAgentPrefix} from '@/lib/auth-routing';
...
export async function GET(request:NextRequest){
  ...
  const brain=['jolly-town','jolly-trade'].includes(requested)?requested:requested.startsWith(jollyAgentPrefix)&&brainFor(requested.slice(jollyAgentPrefix.length))?jollyAgentPrefix+brainFor(requested.slice(jollyAgentPrefix.length))!.slug:brainFor(requested)?.slug||'gpt';
