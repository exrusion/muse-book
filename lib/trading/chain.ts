...
const makeClient=(url:string)=>createPublicClient({chain,transport:http(url,{timeout:12000,retryCount:1})});
let client:ReturnType<typeof makeClient>|undefined;
export function rpc(){
 if(!client){const url=process.env.JOLLY_TRADING_RPC_URL||chain.rpcUrls.default.http[0];if(!url.startsWith('https://'))throw new Error('HTTPS RPC required');client=makeClient(url);}return client;
}
...
export async function scan(){
 ...
 let from=saved?BigInt(saved.value.block)+1n:(final>1000n?final-1000n:0n);
 if(from>final)return;
 const to=from+99n<final?from+99n:final;
