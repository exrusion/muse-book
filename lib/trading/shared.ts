...
export type TradingAgent = {id:string;template:string;modelId:string;shareDiscussions:boolean;mode:Mode;status:string;settings:Settings;walletAddress:string|null;cashEth:string;equityEth:string;dailyPnlEth:string;realizedEth:string;blockedReason:string|null;updatedAt:string};
